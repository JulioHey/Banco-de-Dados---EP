...
class Cash_outflow(BaseModel):
    ...
    def fromList(list):
        return Cash_outflow(
            ...
            value=list[4],
            quantity=list[5],
            cash_outflow_TYPE=list[6],
            responsible_cpf=list[7],
        )
    def __repr__(self):
        details = '{\n'
        ...
        details += 'cash_outflow_TYPE: ' + self.cash_outflow_TYPE + '\n'
        details += 'responsible_cpf: ' + self.responsible_cpf + '\n'
        details += '}'
        return details

    def insertSql(self) -> str:
        sql = 'insert into cash_outflow values ('
        ...
        sql += ','
        sql += '"{}"'.format(self.cash_outflow_TYPE) if self.cash_outflow_TYPE else 'NULL'
        sql += ','
        sql += '"{}"'.format(self.responsible_cpf) if self.responsible_cpf else 'NULL'
        sql += ');'
        return sql
    ...
    def querySql(where: dict, attr: list = []) -> str:
        if len(attr) == 0:
            attr = ['payment_form', 'tax', 'emission_date', 'payment_id', 'value', 'quantity', 'cash_outflow_TYPE', 'responsible_cpf']
        sql = 'select {} '.format(','.join(attr))
        sql += 'from cash_outflow '
        if len(where.keys()):
            sql += "where "
